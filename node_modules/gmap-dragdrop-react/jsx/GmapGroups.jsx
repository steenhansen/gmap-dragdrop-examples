...
class GmapGroups extends GmapDragDrop {
  ...
  _onIdle_googleListener(e) {
    const first_time_idle = GmapDragDrop.prototype._onIdle_googleListener.call(
      this,
      e
    );
    if (first_time_idle) {
  ...
  drawPolyline(pin_color) {
    ...
    let colored_path = this._drawMembers(
      group_lat_lng,
      activity_locations,
      pin_color
    );
  ...
  _clearPolyline(pin_color) {
    if (
      this._gmapGroup_vars.group_polylines[pin_color] !== undefined &&
      this._gmapGroup_vars.group_polylines[pin_color] !== null
    ) {
  ...
  deleteMember(lat_lng_obj) {
    var colored_locations = this.filterByPropertyValue(
      "pin_color",
      lat_lng_obj.pin_color
    );
  ...
  _onAddGroup(e) {
    ...
    const { location_data } = e.gmap_params;
    this._gmapGroup_vars.next_location_id =
      this.getOutingDistance(location_data);
    if (typeof location_data.lat === "function") {
      try {
        throw new Error(
          "Cannot add single locations in GmapGroups, only arrays"
        );
      } catch (e) {
        console.log(e);
      }
