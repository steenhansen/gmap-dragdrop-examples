...
const PIN_SVG_DEFAULT = "m256.5,512c-25.5,-255 -127.5,-280.5 -127.5,-382.5a127.5,127.5 0 1 1 255,0c0,102 -102,127.5 -127.5,382.5zm-25.5,-382.5a25.5,25.5 0 1 1 51,0a25.5,25.5 0 1 1 -51,0"
...
// auto	"use strict"
  ...
  static browserFactory(container_id, gmap_properties) {
    ...
    const gmap_component = gmap_factory(gmap_properties)
  ...
  _latLngDrop(drop_event) {
    const scrolled_y = document.documentElement.scrollTop || document.body.scrollTop
    const scrolled_x = document.documentElement.scrollLeft || document.body.scrollLeft
    ...
    let lat_lng_func = this._convertXyToLatLng({x: marker_pos_x, y: marker_pos_y})
  ...
  _convertXyToLatLng(mouse_pos) {
    ...
    const top_right = google_map.getProjection().fromLatLngToPoint(google_map.getBounds().getNorthEast())
    const bottom_left = google_map.getProjection().fromLatLngToPoint(google_map.getBounds().getSouthWest())
  ...
  _onDragStart_react(drag_event) {
    ...
    if (location_id) {
      ...
      const canvas_element = this._canvasPin(json_parameters.pin_color, json_parameters.title_text, json_parameters.content_text)
  ...
  _makeDragParameters(location_id, drag_event) {
    let location_data = Object.assign({}, this._gmapDragDrop_vars.location_datas[location_id], {"from_id": this._gmapDragDrop_vars.container_id})
    if (this.state.map_options.onDragStartMarker !== undefined) {
      const event_parameters = this._eventParameters('location_data', location_data)
  ...
  _onDragEnd_react(drag_event) {
    ...
    if (this.state.map_options.onDragEndMarker !== undefined) {
      const event_parameters = this._eventParameters('from_location', from_location)
      let extended_event = Object.assign({}, drag_event, event_parameters)
  ...
  componentDidMount() {
    ...
    this._addSvgToBody()
    this._canvasSvg()
    this._gmapDragDrop_vars.container_id = ReactDOM.findDOMNode(this).parentNode.getAttribute("id")
  ...
  _initGmap() {
    ...
    let {scroll_wheel, gestureHandling, init_zoom, map_type, street_view, zoom_control, map_type_control, map_styles}=this.state.map_options
    const self = this
  ...
  _canvasPin(drag_marker_color, title_text, content_text, pin_scale = this.state.map_options.pin_scale, canvas_id = DRAG_CANVAS_ID) {
    let canvas_element = document.getElementById(canvas_id)
    let context = canvas_element.getContext("2d")
    ...
    const pin_svg_path = new Path2D(this.state.map_options.pin_svg)
  ...
  render() {
    ...
    return (
            <div id={this.state.REACT_DIV_ID}
              ...
              <div id={this.state.GOOGLE_DIV_ID}
                   style={H_W_100_REL_STYLE}></div>
              {start_drag_function}
              {canvas_ie_svg}
            </div>
    )
  ...
  _canvasIeSvg() {
    ...
    const ie_drag_pin_width = PIN_SVG_H_W * this.state.map_options.pin_scale + 'px'
    const svg_style = {
      position: 'fixed'
      , zIndex: 0
  ...
  _onMouseMoveOverlay(event) {            /// drag 2
    if (this._gmapDragDrop_vars.mouse_is_up) {
      const mouse_out_x = Math.abs(this._gmapDragDrop_vars.last_marker_x - event.pageX)
      const mouse_out_y = Math.abs(this._gmapDragDrop_vars.last_marker_y - event.pageY)
  ...
  _onDragOver_react(drag_event) {
    if (this._gmapDragDrop_vars.am_dragging) {
      ...
      if (this.state.map_options.onDragMarker !== undefined) {
        const event_parameters = this._eventParameters('start_lat_lng', start_lat_lng, 'location_id', location_id,
                'page_x', drag_event.pageX, 'page_y', drag_event.pageY)
        let extended_event = Object.assign({}, drag_event, event_parameters)
  ...
  _onMouseLeftDownOverlay(event) {     /// drag 3
    const mouse_out_x = Math.abs(this._gmapDragDrop_vars.last_marker_x - event.pageX)
    const mouse_out_y = Math.abs(this._gmapDragDrop_vars.last_marker_y - event.pageY)
    if ((mouse_out_x < ALLOWABLE_MARKER_DRIFT) && (mouse_out_y < ALLOWABLE_MARKER_DRIFT)) {
  ...
  _mouseOverMarker_googleListener(map_marker, lat_lng_obj) {          /// drag 1
    let self = this
  ...
  _onMouseRightDownOverlay(event) {
    ...
    if (this.state.map_options.onRightClickMarker !== undefined) {
      ...
      const location_id = this._locationIdForLatLng(start_lat_lng)
      if (location_id) {
        const event_parameters = this._eventParameters('location_id', location_id)
  ...
  locationDelete(location_id, moving_marker = '') {
    let delete_location_data = this._gmapDragDrop_vars.location_datas[location_id]
    if (delete_location_data.can_delete || moving_marker === INSIDE_TO_INSIDE_MOVE) {
      ...
      if (this.state.map_options.onDelete !== undefined) {
        const event_parameters = this._eventParameters('location_data', delete_location_data)
  ...
  _popUpWindow(map_location, map_marker) {
    const {title_text, content_text, info_style, title_style, content_style, showing_info_window, extra_text}=map_location
  ...
  _placeMarker(partial_map_location) {
    ...
    try {
      ...
      let pin_path = this.state.map_options.pin_svg
      if (map_location.marker_svg) {
        pin_path = map_location.marker_svg
      }
      let marker_icon = {               // must have a pin path for drag image....
        path: pin_path
  ...
  _outsideToInsideDrop(location_id, id_lat_lng_title_content_obj) {
    this._hideIeImageDrag()
    let changed_location = this.addChangeEvent(id_lat_lng_title_content_obj)
    if (changed_location) {
      if (this.locationExists(location_id)) {
        let old_location_data = this._gmapDragDrop_vars.location_datas[location_id]
  ...
  addChangeEvent(lat_lng_obj) {
    let changed_lat_lng_obj = Object.assign({}, lat_lng_obj)
    if (this.state.map_options.onAdd !== undefined) {
      let event_parameters = this._eventParameters('location_data', changed_lat_lng_obj)
  ...
  _onCenterChanged_googleListener() {
    if (this._gmapDragDrop_vars.on_ready_fired) {
      ...
      if (this.state.map_options.onCenterChanged === undefined) {
        ...
        const event_parameters = this._eventParameters('moved_center', moved_center)
        const move_allowed = this.state.map_options.onCenterChanged(event_parameters)
        if (move_allowed) {
          this._gmapDragDrop_vars.center_lat_lng = moved_center
        } else {
  ...
  _onZoomChanged_googleListener() {
    if (this._gmapDragDrop_vars.on_ready_fired) {
      ...
      if (this.state.map_options.onZoomChanged === undefined) {
        ...
        const zoom_allowed = this.state.map_options.onZoomChanged(event_parameters)
        if (zoom_allowed) {
